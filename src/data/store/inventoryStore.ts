...
export function createInventoryStore(db: FieldShoreDB): InventoryStoreApi {
  ...
  async function addOne(spec: AddSpec): Promise<string> {
    const result = await db.transaction('rw', db.inventory, async () => {
      ...
      if (match) {
        const updated = { ...match, quantity: match.quantity + 1, available: match.available + 1 };
        await db.inventory.put(updated);
        return updated;
      }
      const created: InventoryItem = { id: `inv-${newId()}`, quantity: 1, available: 1, ...spec };
  ...
  async function incrementItem(id: string): Promise<void> {
    const result = await db.transaction('rw', db.inventory, async () => {
      ...
      const updated = { ...item, quantity: item.quantity + 1, available: item.available + 1 };
      await db.inventory.put(updated);
      return updated;
    });
    applyLocal(result);
  ...
  async function decrementItem(id: string): Promise<void> {
    ...
    if (outcome.kind === 'removed') removeLocal(id);
    else if (outcome.kind === 'updated') applyLocal(outcome.item);
  }
  ...
  async function setQuantity(id: string, quantity: number): Promise<void> {
    const outcome = await db.transaction('rw', db.inventory, async () => {
      ...
      const updated = { ...item, quantity: q, available: q - deployed };
      await db.inventory.put(updated);
      return { kind: 'updated' as const, item: updated };
    });
    if (outcome.kind === 'removed') removeLocal(id);
      ...
      for (const r of rows) {
        ...
        if (r.id) {
          ...
          if (existing) {
            ...
            await db.inventory.put({ ...fields, id: r.id, quantity: r.quantity, available: r.quantity - deployed });
          } else {
            await db.inventory.add({ ...fields, id: r.id, quantity: r.quantity, available: r.quantity });
          }
        } else {
          await db.inventory.add({ ...fields, id: `inv-${newId()}`, quantity: r.quantity, available: r.quantity });
        }
        imported++;
      }
      if (newRigs.size > 0) {
        await db.meta.put({ key: APPARATUS_ROSTER_KEY, value: JSON.stringify(nextRoster) });
      }
    });
    await boot();
