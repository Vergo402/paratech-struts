  ...
  return (
    <div className="fs-spd">
      ...
      <section className="fs-spd-section">
        <div className="fs-spd-idrow">
          <Badge variant="status" status={sp.status} />
          {sp.seq != null && <span className="fs-spd-seq">#{sp.seq}</span>}
        </div>
        <p className="fs-spd-where">{where || SHORE_TYPE_LABELS[sp.shoreType]}</p>
        {sp.assignedResource && <p className="fs-spd-assigned">Assigned: {sp.assignedResource}</p>}
        {bomModelLabel(sp) && <p className="fs-spd-strut">{bomModelLabel(sp)}</p>}
      </section>

      {/* B + D — measurement & deduction ledger */}
      <section className="fs-spd-section">
        ...
        <div className="fs-rec-ledger">
          ...
          {hasDeductions && ledger.map((row) => <LedgerSlot key={row.label} row={row} />)}
          <div className="fs-rec-row fs-rec-effective-row">
            <span className="fs-rec-slot-label">
              {sp.status === 'secured' || sp.status === 'returned' ? 'Set length' : 'Effective length'}
            </span>
      ...
      <section className="fs-spd-section">
        ...
        {/* Anything but a confirmed pass is safety-relevant — a flagged (warn) OR an
            unverifiable (unknown) assembly both announce assertively; only the clean
            "within rated" pass stays quiet. Never a false pass. */}
