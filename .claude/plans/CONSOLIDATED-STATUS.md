...
### v3.20.0 ✅ shipped 2026-05-25 — External Department Equipment inventory plumbing (#127)

External (visiting-dept) equipment is now fully deployable AND visible in the Quick View sidebar. Two-part fix: (1) the deploy/return pipeline had asymmetric external branching — the strut path already mirrored on `.external` at four sites since v3.11.x, but the extension path didn't at any of them, and plates had the same gap downstream (find worked via `opInv` but decrement/compensate/transaction/return all used `localInventory.find` which returns undefined for external). External extensions were silently invisible at deploy time; external plates were silently lost on return since v3.17.0 when the external-equipment form rebuilt landed. v3.20.0 mirrors the strut path's pattern at 22 sites (12 extension + 10 plate) — same conditional shape, same Firebase ref routing (`operationsRef/externalEquipment/{id}/available` + `ext:{opId}:{id}` epoch keys). (2) New **External Department Equipment** mega-section in `renderQuickViewInventory()`, visually distinct (4px double blue rule + uppercase header), with per-apparatus sub-headers ("Yonkers FD / SQ-11") and items grouped underneath using the same row format as dept-owned blocks above (item name + system label `Grey`/`Gold`/`Plate` + count).

GATE 2 review: code-auditor caught 3 BLOCK-class findings (deploy/return asymmetry — `runDeployTransactions` ext branch, optimistic decrement synthetic-copy issue, `returnInventoryItems` lost external on return); battalion-chief asked for provenance subordination + Firebase sync confirmation; mobile-ux flagged a runner-color collision on the original blue-pill EXT badge design and required truncation rules + a11y attrs; skeptical-senior-engineer's plate-path expansion catch (same class, pre-existing prod bug) made v3.20.0 the right vehicle to close both at once. Two design iterations during ship: initial design had a blue EXT pill on each row + dept/apparatus suffix → user pushback ("remove the blue box with EXT") collapsed to just the dept/apparatus suffix → second pushback ("system labels not dept names; group by apparatus like dept-owned") restructured to per-apparatus sub-headers matching the dept-owned format. **Lesson:** review reviewers can't anticipate every user-facing UX preference — the GATE 2 pass surfaced correctness gaps cleanly but the visual design still iterated twice with the user after implementation. For future MINORs, consider a "design pass" step between GATE 2 approval and final implementation that walks through the actual rendered UI mockup, not just the spec language. **#80 (concurrent-deploy phantom-double)** re-labelled to v4.0.0 — full fix is Cloud Function gated on per-device UID auth. Plan: [archive/v3.20.0-external-eq-plumbing.md](archive/v3.20.0-external-eq-plumbing.md).
...
### v3.22.0 ✅ shipped 2026-06-07 — inventory integrity + measurement safety (3 items)
Shipped as a focused integrity release after **both planned features were descoped during the build**: #103 (header/footer beams) → Backlog, #123 (Command breadcrumb) → deferred to the v4 Command redesign (it needs a multi-dept model v3 lacks; building it on free-text risks a misleading IC accountability picture). Final scope: **#300** — measurements now floor to 1/8″ (round DOWN; short is taken by the loading wedge, long is the hazard; ADR-012 backport). **#80** — concurrent-deploy phantom-double, **descoped at GATE 2** to the cheap provable fixes: clamp the unclamped partial-abort compensation (no more inflating `available` past `quantity`), numeric `.validate` for external-equipment counts, and delta-reconciliation (`{baselineAvail, delta}`) across **both** offline touched-pipes so a concurrent offline double-deploy **surfaces** as a persistent "VERIFY PHYSICAL STRUT" SP badge (IC/Safety-ack, excluded from secured counts) instead of silently converging to 0; the full allocation-ledger + Cloud Function deferred to v4. **#284** — behavior-preserving DRY of the inventory deploy/return mutation sites into 4 helpers (post-#80 contract; two commits never squashed). 7-agent GATE-2 review (all CONCERNS) folded in. Plan: [v3.22.0-beams-breadcrumb-integrity.md](v3.22.0-beams-breadcrumb-integrity.md). **Lessons:** (1) forward-compat schema (#80's ledger) doesn't belong on the v3 line v4 replaces — descope to provable fixes, defer architecture to where the multi-device model lands; (2) when features are trimmed mid-build, a partial agent edit (here the #103 beam constant) can linger uncommitted in the working tree — caught at the version-bump step before it shipped. Watch for it on future descopes.
