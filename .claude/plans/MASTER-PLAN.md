...
| **v3.17.0** | MINOR — local-first defaults (scenario presets, solo-IC, Quick-start FAB) + role-hierarchy migration + drilldown-search v2 + pre-v4 dual-write window (`Strut Placed`→`Strut Set`, `customRoles`→keyed, `assignedApparatus`→keyed) + #71 architectural full-fix | 1-2 weeks | Low | Onboarding + v3.16.x carry-overs + schema migration prep | ⏳ Planned |
