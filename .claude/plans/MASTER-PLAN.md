...
> **Status as of 2026-05-19:** v3.5.2 → v3.14.3 shipped (per-release detail catalogued below; full table in `CONSOLIDATED-STATUS.md`). **v3.15.0** ✅ shipped (numbered divisions #93 + offline inventory hardening — #71 mitigated via flush-pass, #80 partial via `makeAllocateDecrementer`). **v3.16.0** ✅ shipped (SmartArt ICS org chart #95 + 12 must-fix items #96 + lock-by-default + ↑↓←→ arrows + ICS-doctrine role hierarchy + 44px touch targets). **v3.16.1** ✅ shipped (permanent inventory right rail on desktop Operations). **v3.16.2** ✅ shipped (Operations legend + remove buggy drilldown search). **v3.17.0** ✅ shipped (solo-IC, FAB, scenario presets, ext-equip resync, schema dual-writes, role/permission cleanup — 12 items). **v3.17.1–v3.17.2** ✅ shipped (FAB hold duration + arc animation mobile fix). Current production: **v3.17.2**.
>
...
| **v3.16.0** | MINOR — SmartArt ICS org chart (#95) + 12 must/should-fix items (#96) + lock-by-default + ICS-doctrine role hierarchy | 1 day | Low | Interactive hierarchy tree with per-node controls | ✅ Shipped 2026-05-19 |
| **v3.16.1** | PATCH — permanent inventory right rail on desktop Operations (fixes floater overlap from v3.16.0) | <1 day | Low | Desktop Operations layout fix | ✅ Shipped 2026-05-19 |
| **v3.16.2** | PATCH — Operations legend (card colors + per-action role permissions) + remove buggy drilldown search | <1 day | Low | Field-feedback UX | ✅ Shipped 2026-05-19 |
| **v3.17.0–v3.17.2** | MINOR — local-first defaults (scenario presets, solo-IC, Quick-start FAB) + role-hierarchy migration + pre-v4 dual-write window (`Strut Placed`→`Strut Set`, `customRoles`→keyed, `assignedApparatus`→keyed) + ext-equip resync + role/permission cleanup (12 items). v3.17.1: FAB hold duration. v3.17.2: FAB arc animation mobile fix. | 1-2 weeks | Low | Onboarding + v3.16.x carry-overs + schema migration prep | ✅ Shipped |
...
**v4.0 release-train duration:** ~3-5 weeks remaining (v3.17.0 + v4.0.0). v3.15.0 and v3.16.x both shipped 2026-05-19. The original v4.0 plan called for v3.12.0 to absorb the local-first defaults; in practice v3.12.0 absorbed the Command-tab refactor, the defaults moved to v3.16.0, then got pushed again to v3.17.0 when v3.16.0 absorbed the org-chart work.
