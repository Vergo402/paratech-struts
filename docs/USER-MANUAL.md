...
- **Top nav bar.** The four tabs (Quick Find, Operations, Inventory, Settings) move from the bottom of the screen to a horizontal bar at the top.
- **Wider canvas.** The app expands to 1200px wide instead of the phone-width 600px column.
- **Split Operations tab.** Your roster (Apparatus, External Equipment, Individuals, My Role) stays pinned on the left while the Shore Points list, Command view, or Cut Table scrolls on the right. The view-switcher tabs continue to work the same way — they just affect the right pane.
...
|---------|------|------------|
| **v3.13** | 2026-05-18 | Desktop view. On screens 1024px wide and up the app now uses a horizontal top nav bar and widens the container to 1200px. The Operations tab splits into two columns on desktop — roster on the left (sticky), Operations / Command / Cut Table on the right. A small toggle in the upper-right of the viewport flips between desktop view and mobile view; the choice is remembered. Phones and iPad portrait are unchanged. |
