...
Total: 247 recommendations. **218 accepted / 13 deferred / 15 rejected / 1 merged.** (Updated 2026-05-31 after Alex's PR #282 review; reconciled 2026-06-07 by the A–E traceability audit — the Lucide reversal recorded, three "Phase E deliverable" docs re-scoped/done, org-chart / dashboard / pocket-lock clarified as Phase F. Counts unchanged.)
...
| 01-architecture | A-30 | Document architecture in docs/v4-design/07-design-system/architecture.md at picker doctrine depth before Phase H. | accepted | **Re-scoped to Phase H (2026-06-07).** Documents the *code* architecture (modules / build), which doesn't exist until the build — essay 01 itself says "before Phase H code starts." Tracked in `99-open-questions.md`; never in the Phase E definition-of-done. |
...
| 02-visual-language | B-5 | Accent #D4A017 (dark theme) and #B8860B (light theme); document contrast ratios. | accepted | Phase E delivers ([`color.md`](07-design-system/color.md) / ADR-011). **Light accent corrected #B8860B → #8C6700** — #B8860B computed 3.01:1 (hard AA fail); #8C6700 = 4.79:1. Dark #D4A017 kept (documented 6.96:1). |
...
| 02-visual-language | B-13 | Custom SVG icons at 24px grid, 1.5pt stroke, 2px corner joins; USACE shore-type diagrams as priority commission. | accepted | Phase E authors a **fully custom** set ([`iconography.md`](07-design-system/iconography.md)) — B-13's custom-SVG path is what shipped. J-9's "Lucide for the general set" is **superseded** (see J-9, revised). |
| 02-visual-language | B-14 | "P" mark geometry refresh; 12pt container radius; two color variants only. | accepted (revised) | **Superseded by ADR-013** — Open Q#10 resolved to a **new full-color identity** (FS emblem + mono mark), not a "P" refresh and not two-color. See [`logo-and-mark.md`](07-design-system/logo-and-mark.md). |
...
| 03-ic-workflow | C-7 | Op period number + elapsed time in persistent header on every IC screen; subtle amber at T-30min; non-blocking transition card at boundary; tag every write with opNumber. | accepted (built #395; opNumber → derived, [ADR-039](11-decisions/ADR-039-derived-operational-periods.md)) | Header clock + rollover card + audit "by period" shipped 2026-06-28. Period DERIVED from boundary events, not stamped on every write. |
...
| 04-future-scale | D-10 | Add opNumber tag on every write — shore points, status transitions, role assignments, inventory transactions. | accepted — mechanism superseded by [ADR-039](11-decisions/ADR-039-derived-operational-periods.md) (#395) | Intent (per-OP export/filter) kept; period is DERIVED from boundary events via `periodOf`, not stamped on every event. Phase J trace flag. |
...
| 07-field-conditions | G-13 | Pocket lock: manual button + proximity sensor (5s covered trigger); swipe-up from bottom handle to dismiss. | accepted | **Phase F/H scope (2026-06-07):** pocket lock is a screen-level interaction (proximity + swipe-dismiss), not a reusable primitive — no Phase E spec. Carried to Phase F IA + Phase H wiring; tracked in `99-open-questions.md`. |
...
| 08-skeptical-review | H-6 | Drop marketing site from v4 scope entirely. | accepted | Accepted per Alex review — the marketing site is dropped from v4 entirely. Any credibility content lives in the user manual. **Amended 2026-07-11 (Alex, #442):** one standalone welcome page (`/welcome.html`, J-21 multi-page entry beside the app) is in scope; app cold-open unchanged — see synthesis §3.3 addendum. |
...
| 10-implementation | J-9 | Adopt Lucide for icons; custom shore-type silhouettes separate. | accepted (revised) | **Revised at Phase E:** [`iconography.md`](07-design-system/iconography.md) adopts a **fully custom** set and rejects every off-the-shelf library incl. Lucide (foreign-grammar argument). The *intent* (a real icon set + separate custom shore-type diagrams) ships; the *means* (Lucide) does not. |
...
| 11-scenario-stress | K-3 | Org chart primitive renders only populated roles by default; grows on demand; never asks IC to manage empty slots. | accepted | **Phase F scope (2026-06-07):** the org chart is a screen-level composition (cards + tree layout), not a reusable primitive — no standalone Phase E spec; the org-card surface is covered by [`card.md`](03-primitives/card.md). Carried to Phase F IA with K-12/K-13; tracked in `99-open-questions.md`. |
...
| 11-scenario-stress | K-9 | Dashboard primitive uses progressive density across four surfaces, not one dashboard with smaller text. | accepted | **Phase F scope (2026-06-07):** the dashboard (SitStat) is a screen, not a reusable primitive; the progressive-density adapters are Phase F IA. Carried forward; tracked in `99-open-questions.md`. |
...
| 12-tech-debt | L-1 | Codify app.js seams in docs/v4-design/07-design-system/module-boundaries.md before Phase H starts. 11 modules. No file > 800 lines. | accepted | **Re-scoped to Phase H (2026-06-07)** — documents app.js's module seams for the Phase H migration; essay 12 says "before Phase H starts." Tracked in `99-open-questions.md`. |
...
| 12-tech-debt | L-3 | Keep v3.9.0 escapeAttr discipline as historical reference in docs/v4-design/LESSONS.md. | accepted | **Done 2026-06-07** — [`LESSONS.md`](LESSONS.md) written (Lesson 3 carries the escapeAttr discipline). |
...
| 12-tech-debt | L-23 | Write docs/v4-design/LESSONS.md capturing institutional memory of every audit-driven fix. | accepted | **Done 2026-06-07** — [`LESSONS.md`](LESSONS.md) written (10 load-bearing lessons; extensible). |
