...
> Source: [`20-operations.md`](../08-information-architecture/20-operations.md) (Operations board — Shore Secured lane, Strut Equipment Returned lane, role gates, Remove & Return modal); [`card.md`](../03-primitives/card.md) (ShorePointCard — `secured` and `returned` states, deployed-strut identity, terminal state); [`modal.md`](../03-primitives/modal.md) (inventory-consequential confirm); [`slider.md`](../03-primitives/slider.md) (Runner → Shore Secured advance; step-back from Shore Secured); [ADR-010](../11-decisions/ADR-010-status-commit-model.md) (always-reversible up to Shore Secured; terminal at Returned); [ADR-016](../11-decisions/ADR-016-modal-vs-sheet-rules.md) (Remove & Return = inventory-consequential modal).
...
**Shore Secured** (strut in the opening, shoring confirmed) and **Strut Equipment Returned**
(equipment physically removed and returned to apparatus inventory at end of engagement).

**Goal:** runner advances Shore Point to Shore Secured. Later — typically at operation end —
...
- **Shore Secured** happens in the field when the strut is confirmed set.
...
| Actor | Surface | When |
|---|---|---|
| **Runner** | Phone (primary) | Advances Runner → Shore Secured on arrival at the opening |
| **Entry / Rescue / Shoring Group Supervisor** | Phone (primary) | Advances Shore Secured → Strut Equipment Returned at equipment retrieval |
...
### Step 1 — Runner → Shore Secured (slide)
...
After the slide: card moves to Shore Secured lane. The Remove & Return action appears on the
card; the advance slide is gone (Shore Secured's only forward transition is via the Remove &
...
Step-back slide on the Shore Secured card — runner realizes the strut is not fully set.
Returns SP to Runner state; no inventory change; no confirm needed. Card returns to Runner lane.

---

### Step 2 — Remove & Return Equipment (Shore Secured → Strut Equipment Returned)
...
**Cancelled →** no state change; returns to Shore Secured card.
...
|---|---|---|
| Runner's **phone** | 1 | Advances Runner → Shore Secured |
| Officer's **phone** | 2 | Taps Remove & Return on Shore Secured card; confirms the modal |
| IC's **tablet** | — | On next sync: Shore Secured lane decrements; Returned lane increments; inventory available count for LS 203 increments in Inventory tab |
| **Broadcast** | — | On next sync: Shore Secured and Returned counts update |
...
|---|---|---|
| Runner → Shore Secured | Yes — step-back | Step-back slide on Shore Secured card (→ Runner) |
| Shore Secured → Strut Equipment Returned | **No — terminal** | Confirm modal states "This cannot be undone." |

No timed undo at any point (ADR-010). Shore Secured is the last reversible state. Once equipment
...
- [`20-operations.md`](../08-information-architecture/20-operations.md) — Shore Secured lane,
...
- [`slider.md`](../03-primitives/slider.md) — Runner → Shore Secured advance; Shore Secured →
  Runner step-back.
...
- [`button.md`](../03-primitives/button.md) — Remove & Return action button on Shore Secured card.
...
- **Shore Secured advance:** **"Shore point Shore Secured. Div 1, Area A, T-Shore, LS 203."**
  (`aria-live="polite"`)
