...
| Elevation | A real cast shadow — modals and sheets are the only primitives granted one | [`color.md`](../07-design-system/color.md) §Strokes & elevation / [`spacing-grid.md`](../07-design-system/spacing-grid.md) §Elevation (the bottom-anchored `--shadow-sheet` geometry is wrong for a centered modal — see Open questions) |
...
1. **`--shadow-modal` is not yet minted.** The modal needs a *centered* cast shadow; the existing `--shadow-sheet` is a bottom-anchored geometry (`0 -4pt 24pt …`) wrong for a surface that sits at center. Mint `--shadow-modal` in its owner ([`spacing-grid.md`](../07-design-system/spacing-grid.md) §Elevation / [`color.md`](../07-design-system/color.md) §Strokes & elevation) during the vertical slice. Flagged here; **#184 mints only the shared `--scrim`**, not a second token.
