...
| It is **non-safety feedback** the operator may ignore | …the **`WarningGate`** when it is a *safety disclosure* (unrated zone, over-capacity, the liability disclaimer) that must persist on the result |
...
4. **The `WarningGate` primitive's own spec.** This doc draws the toast / `WarningGate` boundary and points; the gate's persistence, placement on the result card, and dismissal semantics are that primitive's to write (matrix **K-11**). `WarningGate` is **not** one of the fifteen named primitive files (#183–196) — it likely needs its own file added to the Phase E cascade. Flagged for the gate.
