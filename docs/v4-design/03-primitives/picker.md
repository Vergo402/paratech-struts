...
> Copied from Section II of `keen-whistling-pancake.md`. This file serves a double purpose: (1) it is the picker primitive spec; (2) it sets the **depth bar** for every other primitive in this folder. When you write `sheet.md` or `card.md` or any other primitive doc, this is the level of detail expected.
...
|---|---|---|---|---|---|
| **Inline segmented** | 2–4 options, mutually exclusive, parent-screen visible at all times | Gold / Grey / LockStroke; Theme (System / Light / Dark); Apparatus tabs | 44pt each, 48pt vertical | None — always visible | n/a |
| **Bottom sheet picker** | 5–7 options, single-select, parent screen stays visible | Base plate, wood size, incident type (NIMS I–V), role-for-assignment | 56pt rows | Slide up from bottom edge over 200ms, scrim fades 0→40% | Backdrop tap, swipe down, system back |
...
Between bottom-sheet picker and full-screen list, the boundary is **8 options**. This comes from Miller's Law (3–7 chunks, 5 optimal, 7 max). At 8 the operator is past the comfortable working-memory span and needs structural help (search, scroll, grouping) rather than a flat list.
...
6. **Sunlight mode** (a brightness + contrast amplification we will spec in Phase E) **does not break picker affordances.** Borders thicken, type weight bumps, shadows drop, but the picker stays a picker.
...
v4 inherits the plate picker's behavior verbatim. The picker doctrine names the "visual grid picker" pattern so future image-card-based selectors (e.g. apparatus thumbnails if we ever add photos) have a consistent reference, but it does not redesign the plate picker. The wood-size selector keeps its grid-with-image affordance.
...
- VoiceOver/TalkBack scripts are documented per variant in `07-design-system/accessibility.md` (Phase E).
- Touch targets meet 44pt minimum on phone, 56pt minimum on tablet/laptop.
...
The picker doctrine is in Phase A — not Phase E where it ultimately lives — to set the **depth bar** for every essay agent and every downstream session. When an agent reads the principles + this picker spec, they understand the kind of work expected. Every primitive will eventually have a doc at this level of detail. Multiply this by ~15 primitives and you have roughly 220 pages of design doc. That is the floor, not the ceiling.
