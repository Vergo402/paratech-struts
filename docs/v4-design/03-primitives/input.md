...
- **Every measurement renders in the value font (Inter, numerals-only), tabular, with diagonal fractions** ([`typography.md`](../07-design-system/typography.md) §Fractions / [ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md)) — plain `"48 1/2″"` text the font composes into a legible diagonal glyph, never the `45¹¹⁄₁₆` codepoint hack. This holds for the value as it is entered *and* as it is read back.
...
[`typography.md`](../07-design-system/typography.md) §Fractions owns the fraction *rule* and explicitly defers the **reusable component** here (`MeasurementValue`). It is the **read-only** counterpart to the measurement field — the same plain-text renderer, used wherever a stored measurement is shown rather than entered:

- Output is **plain text** — `"48 1/2″"`, a space before the fraction — and the value font composes the diagonal glyph via `font-variant-numeric: diagonal-fractions` (set on `.fs-meas`). **Diagonal is the house style** ([ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md)); the hand-stacked `.fr` digit-pair is retired. No glyph codepoints, never the `45¹¹⁄₁₆` hack.
- Inter (value font), `font-variant-numeric: diagonal-fractions tabular-nums`, ⅛″ denominators only (2, 4, 8). Values in a column align on the integer's right edge.
...
| Field value | Free text at **16pt** (the field-readable floor); **measurements & loads in the value font (Inter), tabular + diagonal fractions** | `--type-body-lg` / `--type-mono` — [`typography.md`](../07-design-system/typography.md) / [ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md) |
...
3. **Measurements and loads render in the value font (Inter), tabular, with diagonal fractions** ([`typography.md`](../07-design-system/typography.md) / [ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md)) — entered and read back. Never the codepoint hack, never the retired hand-stacked digit pair.
...
- **The `45¹¹⁄₁₆` codepoint hack, or the retired hand-stacked digit pair, in a measurement.** Use `MeasurementValue` — plain text + diagonal fractions via the value font ([`typography.md`](../07-design-system/typography.md) / [ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md)).
...
1. **Exact measurement-keypad layout.** The ft / in / fraction arrangement, and whether the ⅛″ fraction is an inline strip or a small picker-sheet (8 values sits just past [`picker.md`](picker.md)'s 7-option boundary), is affordance geometry finalized in the **vertical slice (Phase H)** — like the card's slide mechanics ([`card.md`](card.md) OQ1) and the sheet's swipe threshold ([`sheet.md`](sheet.md) OQ2). The *doctrine* (56pt keys, value font + diagonal fractions per [ADR-028](../11-decisions/ADR-028-inter-numerals-diagonal-fractions.md), exact entry / floored cut answer) is fixed here.
