...
| Multi-building operation; Include deductions; Native controls | Theme (System / Light / Dark); strut display (Gold / Grey / LockStroke); apparatus tabs |

> **The v3 cautionary tale:** `.system-toggle` (`style.css:451`) is *named* a toggle but renders three co-equal options — Gold, Grey, LockStroke. There is no "off." It is a **segmented control** and belongs to [`segmented.md`](segmented.md), not here. v4 does not inherit the misnomer. The same applies to Theme — System / Light / Dark / Sunlight is a choice among named themes ([`picker.md`](picker.md) "inline segmented"), never a switch.
...
| Track — off | A **neutral UI-component grey** distinct from both the row surface and the thumb | a neutral cited from [`color.md`](../07-design-system/color.md) (working candidate `--text-tertiary`, the one text token color.md sanctions "for UI components"); finalized in the slice — see Open questions |
...
| `.system-toggle` / `.toggle-dot` / `.toggle-label` — Gold / Grey / LockStroke (`style.css:451`) | **Not a toggle** → segmented control ([`segmented.md`](segmented.md)) |
...
- **A switch for a named multi-option choice.** Gold / Grey / LockStroke and System / Light / Dark are segmented controls ([`segmented.md`](segmented.md)) — the exact v3 `.system-toggle` mistake this doc retires.
...
1. **The off-track neutral fill + exact geometry.** The *vocabulary* is fixed here (capsule track, position-carries-state, on = `--accent`, two variants). The off-track neutral and the pixel dimensions (track width/height, thumb diameter, travel distance) are affordance geometry finalized in the **vertical slice (Phase H)** — like the sheet's swipe threshold (sheet.md OQ2) and the badge's pixel padding (badge.md OQ1). The constraint is recorded: the off-track fill must clear **WCAG 1.4.11 non-text contrast (3:1)** against *both* the row surface and the thumb; `--text-tertiary` is the working candidate (the one color.md sanctions for UI components), to be verified by [`wcag-contrast.mjs`](../07-design-system/wcag-contrast.mjs) in the slice.
