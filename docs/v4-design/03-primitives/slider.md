...
3. **The slide is the ONLY commit path** ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)). No visible Advance / Step-back button twin and no hidden AT/keyboard commit path — gesture-only is a deliberate, recorded accessibility exception (Alex's KB-5 ruling, Phase H gate). A gated slide carries its reason as a **visible text line under the track**, owned by this primitive.
...
| **Laptop (Toughbook)** | **Pointer-drag, same as touch.** There is no keyboard commit path ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)) — the mouse/trackpad drag is the deliberate commit on this surface too. Keyboard users operate everything *around* the slide (navigation, pickers, buttons); the status commit requires the pointer. |
