...
| **Step-back slider** (secondary) | the **previous** lifecycle status | **trailing (right) edge → leftward** | **Secondary** — smaller, lower-contrast, placed below the advance track | "Step back to Cutting Station" |
...
- **`pending → process` is not a slide.** It is **Assign Equipment** — a deploy action that pulls a strut from inventory — rendered as a full-width process-blue button, because reaching Equipment Assigned *means* a strut was deployed ([`card.md`](card.md) §Pending). A point with nothing to advance has no track.
...
The two configurations differ in *geometry*, not just emphasis — the advance is a louder, taller, accent-tinted channel; the step-back is a quieter, shorter, plain-surface mirror — so they never read as the same control (finalized in the slice, S12):

| Part | Advance (primary) | Step-back (secondary) | Token / source |
|---|---|---|---|
| **Track** | a **52px** pill channel, **accent-tinted** (`--accent` mixed **16%** into the card surface) with a 45%-accent hairline | a **44px** pill channel on the **plain card surface** with a `--surface-stroke` hairline, layout **mirrored** (row-reverse) | `999px` pill radius (height ÷ 2, not a `--radius-*` member); 56/60pt floors [`spacing-grid.md`](../07-design-system/spacing-grid.md) |
| **Knob** | a **44px** circle in solid `--accent` (`--on-accent` chevron), inset 4px from the track edge | a **36px** quieter circle on `--surface-elevated` with a hairline border and a secondary-ink chevron | `--accent` / `--on-accent` [`color.md`](../07-design-system/color.md); the chevron is supplemental, never load-bearing |
| **Label** | next step in words, centered, primary weight, **padded clear of the at-rest knob** (`padding-left: 52px` = 44 + 8) | secondary weight + ink, **padded clear of the right-resting knob** (`padding-right: 44px` = 36 + 8) | `--text-primary` / `--text-secondary`; primary-action label treatment [`button.md`](button.md) / [`typography.md`](../07-design-system/typography.md) |
| **Travel reveal** | **kept** — as the knob moves, the channel behind it fills with the **destination status's hue** (`revealColor`), the operator dragging the next state into being | same, mirrored from the right edge | `--status-{next}-*` [`color.md`](../07-design-system/color.md) §status palette (the solid banner fill in sunlight) |

- **The label-clearance rule is now a hard geometry rule** (S12): each variant's label is padded by *its own* knob diameter + the 8pt dead zone, so the text never sits under the knob at rest and never clips behind it. (Surfaced at S11 verification — the slide-track label clipped behind the handle in a narrow 2-col lane (~328px). The knob-side padding is the fix; verify at the re-drive.)
- **The travel-reveal is retained by doctrine** — it is what makes the slide *mean* something (the channel fills with the color of the state you are committing to, and the badge cross-fades to that exact hue on commit). It was never a candidate to drop; OQ3 below is resolved keeping it.

The knob inset (4px from the track edge) and the 999px pill radius are the channel geometry; the **travel-reveal fill** sits above the track tint and below the knob, clipped to the pill by the track's `overflow` + radius. The slider still mints **no token**; it is assembled entirely from the surface, accent, status, and motion vocabularies the system already owns.
...
- **Commit only past a deliberate threshold — finalized at 0.6 of travel** (S12). The slide commits **only when the thumb is dragged past 60% of the available travel** and released there; released short, it snaps back and commits nothing. This threshold is the ghost-tap defense: a gloved brush or a raindrop moves the thumb a little, not across 60%, so nothing commits. 0.6 sits deliberately past the midpoint — far enough that a casual touch can't reach it, near enough that a committed drag isn't a fight. (The control file had left "the exact threshold proportion … finalized in the vertical slice"; the slice settled it. The value is the default of `shouldCommit(offsetPx, trackPx, threshold = 0.6)` in `Slider.tsx`, exported pure for tuning + tests.)
...
3. **On TOUCH the slide is the ONLY commit path** ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)). No visible Advance / Step-back button twin and no hidden AT/keyboard commit path — gesture-only is a deliberate, recorded accessibility exception (Alex's KB-5 ruling, Phase H gate). **On a MOUSE the slide swaps for a single tap-once button instead of the track** ([ADR-035](../11-decisions/ADR-035-mouse-status-commit-button.md), `useHasMouse`) — the ghost-tap risk the slide defends against requires a finger, so on a pointer the button is the right affordance (not a twin: the slide is absent). A gated slide/button carries its reason as a **visible text line under the control**, owned by this primitive.
...
| **Laptop / desktop (mouse)** | **A single tap-once button, not the slide** ([ADR-035](../11-decisions/ADR-035-mouse-status-commit-button.md), `useHasMouse` = `(hover: hover) and (pointer: fine)`). A precise drag-past-threshold is clumsy with a pointer and the wet-glove ghost-tap risk is gone, so the slide is replaced by a `primary` (advance) / `secondary` (step-back) button committing the same event. Fully keyboard- and AT-operable here — it is a real `<button>`. Detection is by **pointer type, not screen width**: a field touch-tablet (wide, coarse pointer) keeps the slide. |
...
**On touch** the slider is the **one recorded exception** to *assistive tech cannot slide* ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md), the Phase H KB-5 ruling): the pointer drag is the only commit path — **no button twin, visible or hidden**. The exception is deliberate, scoped to status transitions, and its trade-off (a VoiceOver / keyboard / switch user observes the lifecycle but cannot drive it) is stated and accepted in the ADR. **On a mouse device that gap is closed**, not widened: the commit is a real `<button>`, fully keyboard- and AT-operable ([ADR-035](../11-decisions/ADR-035-mouse-status-commit-button.md)). What this primitive still owes assistive tech and the field:

- **Everything announces; nothing is operable.** The thumb is `aria-hidden` and out of the focus order — the slide never half-presents as a control a reader can't use. The track label is real visible text ("Slide to set Runner"); a **gated slide renders its reason as a visible text line under the track** ("Waiting on group — 2 of 3 still Pending Equipment"), never a tooltip or a silent lock.
...
- **Commit announces once, politely, as a word.** A `aria-live="polite"` region speaks **"Shore point B-2, now Cutting Station"** on advance and the prior state on reversal — the status *label*, never the color (Principle 9). Grouped commits announce scope: "Advancing all 2 group members."
...
- **Screen-reader script** (registered in [`accessibility.md`](../07-design-system/accessibility.md) §Screen-reader scripts): the **Status slide** row records what a reader *hears* — the visible track/reason text and the polite commit announcements — not an operable path. This file follows that registry; it does not re-invent the grammar.
...
- **A button twin beside (or hidden behind) the slide.** Retired by [ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md): the Phase H gate drive found the doubled control unusable — two affordances for one action, and the button reintroduces the ghost-tappable commit the slide exists to prevent. Do not reintroduce Advance/Step-back buttons, visible *or* AT-only; if an operability remedy is ever needed, it is a new ADR.
...
1. ~~**Exact slide mechanics & commit threshold.**~~ **RESOLVED (S12).** The control is a dedicated slide-toggle on the card (not a full-card swipe), and the commit threshold is **0.6 of travel** (`shouldCommit(…, threshold = 0.6)` in `Slider.tsx`). The rules (deliberate threshold, snap-back, gesture-only commit per [ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)) were already fixed; the slice settled the geometry. Whichever the case, the commit still plays `--motion-status` + a medium haptic.
2. ~~**Track rest fill + thumb pixel geometry.**~~ **RESOLVED (S12).** Advance = 52px accent-16%-tinted pill + 44px `--accent` knob; step-back = 44px plain-surface pill + 36px `--surface-elevated` bordered knob; 999px pill radius, knob inset 4px. See the Anatomy table.
3. ~~**Travel-reveal treatment.**~~ **RESOLVED (S12) — kept.** The destination-status hue fills the channel behind the knob (clipped to the pill), reaching the knob centre as it travels; it matches the commit cross-fade. Retained by doctrine, not dropped.
