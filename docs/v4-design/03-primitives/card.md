...
> **Card compaction (2026-06-20, Alex) — the interior-officer glance card.** Now that the **Details** pop-up ([Quick View](side-drawer.md), #340) holds the full bill of materials, per-rig sources, deduction ledger, load, timeline, and re-verified safety, the card sheds the duplication: (1) the deployed **strut + extensions** (`bomModelLabel`) fold **into the value bar** as a sub-line — the separate strut block is gone — **except at the Cutting Station / `cutting` state, where the bar shows the cut length alone** ("cutting only needs the cut length"); (2) the **assigned resource** (`assignedResource`) appends to the **location line** (`Building · Div · Area · Rescue 2`) — its own line is gone; (3) the **source rig** the strut came from moves **into Details** (off the card face). This **revisits the "deployed strut model + apparatus source on the face, cradle-to-grave" doctrine** above — recorded as a deviation, **flagged for the Phase J doctrine audit** (`98-design-docket.md`).
...
> **Card compaction (2026-06-20, Alex) — supersedes the detail line above.** The `.fs-spc-detail` line (Raw opening + deduction + load) is **retired from the card face** — that context now lives in **Details** ([`ShorePointDetail`](side-drawer.md), "Measurement & load"). In its place the bar runs on **one baseline line: label · measurement · strut** — the measurement sits directly after its label and the deployed **strut + extensions** (`.fs-spc-value-eq`, `bomModelLabel`) ride in line at a comparable size — **except `cutting`, which stays cut-length-only**. To fit that on one line in the narrow board lanes, the **bar label is short** — `Strut system` (pre-cut) / `Cut length` (cutting · runner) / `Set length` (secured · returned), per Alex (the deduction LEDGER below keeps the full "Required strut length" wording — §Ledger vocabulary). The "Detail line above the shelf" column in the table above is therefore historical; the live card has no detail line. Flagged for the Phase J doctrine audit (`98-design-docket.md`).
...
- Shows a **waiting callout** (when there's a reason), then one primary action: **"Assign Equipment"** (a deploy action, full-width, process-blue) — **not** a slide-to-advance. (Faithful to v3, where the pending card shows an Assign Equipment button, never a status slider.) *(Card compaction 2026-06-20: the literal "No equipment assigned" line was dropped — an empty value bar + the Assign action already say it.)*
...
- Keeps the shore-point identity (name, location, the required length in the bar — labelled **"Strut system"**) so the point is actionable the moment equipment arrives. (Raw opening + load moved to Details with the card compaction.)
