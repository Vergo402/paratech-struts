...
| E | Design system (color, type, spacing, motion, primitives, etc.) | 🚦 Gate ready | **Gate trio authored 2026-05-31, rendered + refined with Alex 2026-06-01.** `color.md` + `typography.md` + `03-primitives/card.md`, four-theme contrast-verified via `wcag-contrast.mjs`. **Living styleguide** added at `preview/` (token-driven HTML, 4 themes × 4 widths, fraction-style toggle) as rendered proof. ADRs: **ADR-010** (status commit model, amends Principle 6) + **ADR-011** (color token system) + **ADR-012** (measurement precision → 1/8″, floor-rounded, SME-confirmed). Card refined through Alex's review: RecommendationCard (1/8″ digit-pair fractions, indented deduction ledger, capacity demoted) and ShorePointCard (deployed strut tracked cradle-to-grave, pending reconciled to the v3 Assign-Equipment model). Awaiting Alex sign-off ([#292](https://github.com/Vergo402/paratech-struts/issues/292)) before the ~19-file cascade (spacing → motion → primitives → cross-cutting). |
