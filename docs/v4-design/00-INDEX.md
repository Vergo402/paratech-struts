...
- `03-primitives/button.md` 🟢 (#187 — the action primitive: **button-vs-not-a-button as a rule** (nav tab → navigation, `.system-toggle`/`.qty-btn`/`.theme-option` → `segmented.md`, `±` stepper → `input.md`, slide-to-advance → `card.md`, link vs button); 32 v3 button classes → **3 emphasis levels + 1 destructive overlay** (primary / secondary / tertiary + danger); primary fills with the **one accent gold** — v3's blue/green/red console palette retired, emphasis is fill→outline→text; **flags `--on-accent`** for `color.md` to mint (the per-theme primary-fill foreground, contrast-verified) — its lone token dependency; icon-button = secondary/tertiary only (Principle 9), **FAB / arc speed-dial retired** (Principle 4); `guardClick` → intrinsic double-fire guard; Advance/Step-back = the slide's accessible-equal button; SR scripts registered in `accessibility.md`)
- `03-primitives/list.md` ⚪
- `03-primitives/input.md` 🟢 (#189 — the data-entry primitive: **input-vs-picker as a rule** (*you type an input; you pick a picker* — v3's `<select>` → pickers, checkbox → `toggle.md`, 1×–4× qty → `segmented.md`); **6 variants** — text / text-area / numeric / measurement / search / chip-tag-input; receives two handoffs — the **interactive chip** (`.app-chip`+`×`) from `badge.md` and the **measurement-display fraction component** from `typography.md`; measurement field = 56pt gloved keypad + 1/8″ stacked fractions (ADR-012, essay 07); validation moves v3 toast/`alert()` → inline `aria-invalid` + specific message, never color-alone; reversible edits never confirm (ADR-010); **mints no tokens** — cites `--radius-input` / `--accent` / `--danger` / `--type-mono` / `--motion-micro`; SR scripts registered in `accessibility.md`)
- `03-primitives/toggle.md` 🟢 (#190 — the binary switch: **toggle-vs-segmented** (the v3 `.system-toggle` Gold/Grey/LockStroke is a *segmented*, not a toggle → `segmented.md`) **+ toggle-vs-checkbox** (selection → `list.md` / completion → `nested-checklist.md`) as a rule; 2 variants — setting toggle (immediate + reversible, Principle 6) / form toggle (staged, commits with its form); **state carried by thumb position, never color alone** (Principle 9); off is the safe default (Principle 5); flip = `--motion-micro` slide+fill; **mints no tokens** — cites `--accent`/neutral/`--surface-card`/`--motion-micro`/56·48pt row; OS-native-switch fallback; SR script registered in `accessibility.md`)
