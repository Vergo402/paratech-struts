...
- **Large TV (passive)** — screen-share target from a tablet or laptop. Display only, never interactive. Designed for legibility at 8–12 feet so the entire CP can see one canonical board.

Every workflow has a story across these four surfaces.

*Rejected:* phone-only design where tablet/laptop are afterthought, or desktop-first dashboards with mobile bolted on.

---

## 3. Calm in chaos.

The app lowers cognitive load, doesn't add to it. No flashing reds, no anxious alarms, no excessive motion. Confidence-grade typography, controlled palette, measured animation.
...
*Rejected:* kitchen-sink toolbars.
...
*Rejected:* modal-stacking. Apple has solved this with Mail's "Undo Send" — we adopt the pattern.
...
No splash screens longer than 400ms. No marketing in the product. No "tip of the day." No "tutorials" between the user and the work.
...
- **Every essay (Phase C) starts from these.** Agents may push back but must explicitly cite which principle they're challenging.
