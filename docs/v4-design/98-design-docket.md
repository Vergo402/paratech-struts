...
| **Spec vs. reducer conflict — cutting→strutset step-back scope.** [`13-cutting.md`](09-workflows/13-cutting.md) §"Step-back out of Cutting" says a grouped step-back from Cutting → Strut Set is **group-wide**; the shipped reducer treats `cutting` as `INDIVIDUAL_PHASE` so the step-back moves **only the triggered card** (`core/operation/reducer.ts` groupAdvance — deliberate, ADR-gated per its own comment). Session 1 followed the reducer (individual). Needs an Alex ruling: amend the spec, or ADR the reducer to broaden it. | ad hoc — **ruling owed** | 2026-06-17 (Session 1) |
