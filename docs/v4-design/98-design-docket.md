...
| **Single-device command-transfer accept on the initiating device** — Step 4-P of [`20-role-assignment-command-transfer.md`](09-workflows/20-role-assignment-command-transfer.md) sketched the pending states as strictly two-device (Accept only on the incoming IC's device). [#401](https://github.com/Vergo402/paratech-struts/issues/401) adds a **hand-the-tablet accept on the OUTGOING device for named-individual targets** (SIM-IV O-2: the workaround left no transfer record); Alex chose to **amend and bless** (approved mockup, 2026-07-01). Workflow doc amended + [ADR-021](11-decisions/ADR-021-command-transfer-handshake.md) addendum; the one-IC invariant and the device-ref strict path are unchanged. Flag for the Phase J doctrine audit. No new ADR (a surface affordance on the existing handshake, not a new primitive). | [#401](https://github.com/Vergo402/paratech-struts/issues/401) / [ADR-021](11-decisions/ADR-021-command-transfer-handshake.md) | 2026-07-01 |
| **Grouped re-measure overrides the #220 field-lock (D2).** #220 locks a shore's sizing (measurement / deductions / shore-type) once it advances past Pending. The Stage-1.1 false-SAFE fix (audit H3/[#417](https://github.com/Vergo402/paratech-struts/issues/417)) makes a GROUPED re-measure fan the sizing change to **every leg, deployed included**, so one physical shore keeps one length (the divergence bug: a mixed-status group left the set leg at the old length). A one-tap **confirmation** gates it when a leg is already set; the set leg then re-verifies honestly against the new length. Single (ungrouped) shores keep the hard #220 lock. Alex's call (D2, 2026-07-04). Doc-comments in `applyPatch` ([`reducer.ts`](../../src/core/shorepoint/reducer.ts)) + `handleSaveEdit` ([`AddShorePointModal.tsx`](../../src/ui/operations/AddShorePointModal.tsx)). Flag for the Phase J doctrine audit. No new ADR (a field-lock scope revision, not a new primitive). | [#417](https://github.com/Vergo402/paratech-struts/issues/417) / `reducer.ts` `applyPatch` | 2026-07-04 |
...
|---|---|---|
| ~~ICS org chart: layout & visual design pass~~ | [#373](https://github.com/Vergo402/paratech-struts/issues/373) | 2026-07-01 — shipped (PR [#396](https://github.com/Vergo402/paratech-struts/pull/396) merged): SVG connector layer, center-on-IC, full-screen pinch-zoom; #373 closed |
| ~~ICS node-detail modal: UX/flow overhaul~~ | [#374](https://github.com/Vergo402/paratech-struts/issues/374) | 2026-07-01 — shipped (PR [#396](https://github.com/Vergo402/paratech-struts/pull/396) merged): single-flow NodeSheet, surface-adaptive (SideDrawer ≥1024 / bottom Sheet); #374 closed |
