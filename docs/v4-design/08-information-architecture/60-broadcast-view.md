...
> **Transport amendment ([ADR-040](../11-decisions/ADR-040-broadcast-cast-transport.md), 2026-07-02).** OQ #34 resolved the *transport* to **OS screen-mirroring** (AirPlay/Chromecast) — and screen-mirroring, left naive, would reintroduce exactly the lurch this section rejects (it copies the caster's live screen). The pin *stability* is preserved a different way: the app ships a **full-screen read-only board mode**, and the operator mirrors a device **parked in that mode** (ideally a spare/dedicated CP device), so the wall holds steady while the primary tablet stays free. The app owns a stable board, not an app-level pin registry; the "don't lurch" contract holds by keeping the mirrored device on the board. See ADR-040 for the full tradeoff (pin-not-mirror is **amended for v4.0**: no in-app cast registry / live-preview-while-working).
...
1. ~~**Cast transport mechanism**~~ — **RESOLVED 2026-07-02 ([ADR-040](../11-decisions/ADR-040-broadcast-cast-transport.md), OQ #34): OS screen-mirroring (AirPlay/Chromecast).** The app ships a full-screen read-only board mode; the operator mirrors a device parked in it (URL-on-TV-browser is the zero-code fallback). Amends pin-not-mirror for v4.0 (see §1 above). The read-only output design is unchanged.
