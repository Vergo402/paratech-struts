...
- **Multi-saw, single-device, shipped in v4.0** ([#354](https://github.com/Vergo402/paratech-struts/issues/354)) — the named-saw model (Option A, auto-claim) works on one device now (one shared cut queue, each saw pulls the top unclaimed cut). Only **cross-tablet live mirroring** is deferred to cloud sync ([#369](https://github.com/Vergo402/paratech-struts/issues/369)). See below.

### Multi-saw — the named-saw model

> **This is the agreed design (Option A), built single-device in v4.0** ([#354](https://github.com/Vergo402/paratech-struts/issues/354)). The named-saw selector, the shared priority-ordered queue, auto-claim, and the muted "on Saw B" markers all work on one device today. What remains deferred to real-time multi-device sync ([#369](https://github.com/Vergo402/paratech-struts/issues/369)) is **cross-tablet live mirroring** — until sync lands, two tablets can't see each other's claims, so the markers reflect only this device's own claims. When sync is live, claims mirror across tablets. Decided by Alex at the [#354](https://github.com/Vergo402/paratech-struts/issues/354) design review.
...
This is **workstation-instancing under Operations** (like running more than one Staging area). It **shipped single-device in v4.0** ([#354](https://github.com/Vergo402/paratech-struts/issues/354)); the **cross-tablet live-mirroring** portion lands with the [#369](https://github.com/Vergo402/paratech-struts/issues/369) sync build.
...
4. **Multiple saw stations (Saw A / B / n…).** ~~Open~~ **Decided and BUILT single-device — the named-saw model (Option A, auto-claim), shipped in v4.0** ([#354](https://github.com/Vergo402/paratech-struts/issues/354)). One shared priority-ordered queue, each saw pulls the top unclaimed cut, each tablet declares its saw (set once), and another saw's active cut shows muted with an "on Saw B" marker — full model in [§Multi-saw](#multi-saw--the-named-saw-model) above. **Cross-tablet live mirroring** is deferred to the [#369](https://github.com/Vergo402/paratech-struts/issues/369) sync build. (Raised by Alex at the #217 gate; resolved at the #354 review.)
