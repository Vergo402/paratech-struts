...
- [x] **No comms / no push** (Principle 10) — **fully in force**; a record of actions, never a messaging surface. The **after-action auto-email** (above) is **not an exception** to it — it's an after-action record read *after* an incident closes (documentation, not communication), **outside the radio rule's scope** ([ADR-018](../11-decisions/ADR-018-after-action-auto-email.md), [#305](https://github.com/Vergo402/paratech-struts/issues/305)).
