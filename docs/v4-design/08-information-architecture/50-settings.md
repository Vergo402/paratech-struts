...
### Phone (the floor)
- **Above fold:** **Appearance** (theme); **Department** (connection / registration + this device's role).
...
- **Primary action (one — Principle 4):** **change a setting** — most **commit in place** (theme, Native Controls) the moment they're toggled, immediate + reversible (Principle 6; the ADR-016 Settings row).
...
- [x] [segmented](../03-primitives/segmented.md) — **theme** (System / Light / Dark — the v3 `.theme-toggle` → the value variant).
...
9. **Administration** — the **gateway links** (navigate elsewhere; this group never holds behavioral toggles — those live in §10). Three gateways, **three different gating rules** (this **resolves [OQ4](#open-questions-per-screen)** and reconciles the [Audit Log](https://github.com/Vergo402/paratech-struts/issues/211) §gateway-gating handoff):
    ...
    | Department policies (§10) | — | **hidden** | visible |

10. **Department policies (Admin-only)** — department-wide **behavioral toggles** (how the app *acts* for the whole department), distinct from §9's navigation gateways. The whole group is **hidden** from anyone who isn't Admin / Manage-department-settings (the hide-not-grey rule).
    ...
    - The group is **extensible** — future department-wide policies (e.g. the "require 2FA" hook, [`99-open-questions.md`](../99-open-questions.md) #33) land here without reworking §9.
...
- Sees: **Appearance · Accessibility · App Updates · Feedback · Reference Materials**.
...
- [x] **Reversible settings never confirm** — theme / Native Controls commit in place (Principle 6); only **Log Out / Leave-Delete Dept** confirm (destructive [`modal`](../03-primitives/modal.md)).
...
- [x] **No roadmap in the product (Principle 11)** — v4.0 ships no Build-choice control and no teaser for the v5 hub; an unbuildable feature is simply absent until it's real, not shown disabled ([`voice-and-tone.md`](../07-design-system/voice-and-tone.md)).
...
| Above fold | Appearance · Department · Accessibility | the same, two-up | same + Excel foregrounded | — |
...
- The theme segmented + the Native Controls toggle announce per the registry ([`accessibility.md`](../07-design-system/accessibility.md) §Screen-reader scripts).
