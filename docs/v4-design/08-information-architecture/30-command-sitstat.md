...
v3 has **no transfer ceremony** — command moves implicitly when a device self-assigns the `ic` role, and the End-Op gate re-checks the live `myRole` each render. v4 adds an **explicit single-action entry**: a transfer affordance **on the persistent IC header** → an **optional full-screen takeover** on the *incoming* device (an ICS-201-style brief, default at Level III+, optional at Level IV–V). It is **a full-screen route, not a stacked modal** (the ADR-016 Command row). The two-device choreography (handshake, who-confirms) is detailed in the Phase G command-transfer workflow. Transfer writes a **role-history** event (K-13).
