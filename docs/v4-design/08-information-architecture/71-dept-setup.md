...
> Source: the master plan **D7.2** (dept ownership — first registrant becomes the founding **Admin** per [ADR-017](../11-decisions/ADR-017-custom-department-roles.md), with "Owner" folded into Admin; generates the first invite code, 24h / one-time); [`06-synthesis.md`](../06-synthesis.md) §1.3 (guest-first, deferrable dept creation) + §Auth-identity (the **one-time founding-Admin claim** — allowed only if the department has no Admin yet; idempotent; the v3→v4 migration path); [ADR-015](../11-decisions/ADR-015-navigation-pattern.md), [ADR-009](../11-decisions/ADR-009-database-firebase-rtdb.md) (per-device UID, security rules), [ADR-008](../11-decisions/ADR-008-nims-org-structure.md), [ADR-016](../11-decisions/ADR-016-modal-vs-sheet-rules.md); the <2-minute zero-config onboarding bar ([`04-references/tablet-command.md`](../04-references/tablet-command.md) anti-pattern). **Net-new** — v3 has no dept-creation UI (a single hardcoded Dept ID via `connectDepartment`, app.js). GitHub [#207](https://github.com/Vergo402/paratech-struts/issues/207).

---

## Purpose

Create a new department (the creator becomes its **first Admin** — [ADR-017](../11-decisions/ADR-017-custom-department-roles.md)), or **claim** an existing un-administered department (the one-time founding-Admin claim, the v3→v4 migration path) — and produce the first **invite code** so teammates can join. The minimal "stand up a department" step, reached forward, deferrable.
...
- **How it is reached:** **forward** from [Login / Register](70-login-register.md) on success when no department exists yet; from [Settings](50-settings.md); or via the **"Claim department" one-time banner** after a v3→v4 migration where no Admin is set. **Never a cold-open gate** ([ADR-015](../11-decisions/ADR-015-navigation-pattern.md)). On success → the shell in AUTHED mode, the new founding Admin.
- **Issue:** [#207](https://github.com/Vergo402/paratech-struts/issues/207).

## Primary role(s) and surface(s)

- **Primary role(s):** the creating user → becomes the **first Admin** (the dept's founding administrator; **Admin is the only built-in role** — [ADR-017](../11-decisions/ADR-017-custom-department-roles.md); titles spelled out, [ADR-008](../11-decisions/ADR-008-nims-org-structure.md)).
...
- **Destructive / terminal:** **none in the common path** (creation is additive; the founding-Admin claim is idempotent and write-once).
...
- [x] [badge](../03-primitives/badge.md) — the resulting **Admin** role badge.
...
## What ships v4.0 (and the founding-Admin claim)

This screen **ships v4.0**. The **founding Admin** is claimed **once** — the security rule permits the claim only if the department has no Admin yet (idempotent; safe to re-run on the migrating device — synthesis §Auth-identity). The first invite code (24h expiry, one-time) is generated here for teammate onboarding via [Invite Code Entry](72-invite-code.md). **Additional Admins** are made later in the [User Manager](https://github.com/Vergo402/paratech-struts/issues/209) (an Admin promotes a member; [ADR-017](../11-decisions/ADR-017-custom-department-roles.md) keeps **≥ 1 Admin** always). A **founding-Admin transfer ceremony** is **not** in v4.0 (deferred v4.1+).
...
- [x] **Device roles spelled out** — "Admin," not an abbreviation ([ADR-008](../11-decisions/ADR-008-nims-org-structure.md), [ADR-017](../11-decisions/ADR-017-custom-department-roles.md)).
...
3. **Founding-Admin transfer** — additional Admins are made via the [User Manager](https://github.com/Vergo402/paratech-struts/issues/209) (the **≥ 1-Admin** guard is [ADR-017](../11-decisions/ADR-017-custom-department-roles.md)); a transfer-of-founding-Admin ceremony is deferred to v4.1+.
