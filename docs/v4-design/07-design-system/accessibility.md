...
**Registry rule.** The primitive cascade's scripts are all in the table above, each following this grammar: `sheet` (#183), `modal` (#184), `badge` (#186), `button` (#187), `list` (#188), `input` (#189), `toggle` (#190), `segmented` (#191), `slider` (#192), `toast` (#193), `empty-state` (#194), `loading-state` (#195), `nested-checklist` (#196), and `warning-gate` (#293 — the 14th file, added at the Phase E audit 2026-06-07 per matrix K-11). (`slider` has no separate row — its assistive-tech equivalent is the **Advance / Step-back control** above, the button path for the slide.) This file is where the scripts are collected, not where their interaction models are invented.
