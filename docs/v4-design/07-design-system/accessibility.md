...
The defining v4 interaction — advancing a shore point's status by a deliberate slide gesture ([ADR-010](../11-decisions/ADR-010-status-commit-model.md)) — is a gesture a VoiceOver or switch user cannot perform. The general rule stands for every *other* gesture in the product: a sheet's swipe-down has a Close button, the side-drawer's edge swipe has its tab, every picker has Power Select — no interaction is gesture-only.

**The status slide is the one recorded exception** ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md), Alex's final KB-5 ruling at the Phase H gate, [#248](https://github.com/Vergo402/paratech-struts/issues/248)):

> **The slide gesture is the ONLY status commit path.** No visible Advance / Step-back button twin (the Phase H gate drive found the doubled control unusable), and **no hidden assistive-tech or keyboard commit path** — a hidden twin is the same doubled model wearing a fig leaf. Status transitions are operable by pointer drag only. The trade-off — a VoiceOver / keyboard / switch user can observe but not drive the lifecycle — is stated and accepted in the ADR, and any future remedy is a new ADR, never a quiet button.

The earlier button-pair contract this section carried (and OQ #37's phone-visible escalation of it) is **retired both directions** by that ruling. What assistive tech keeps, in full: every card, status, and reason is readable in text; the commit announces in both directions through a polite live region — **"Shore point B-2, now Cutting Station"** on advance, the prior state on reversal (the label *word*, never the color — Principle 9); a gated slide shows its reason as visible text under the track; and because the model is *commit-immediately, always-reversible* and not a timed-undo toast, no one is racing a countdown they cannot see (ADR-010). Every consequential **action** — deploy, return, End Operation — remains a fully operable button or modal; the exception covers status transitions and nothing else.
...
| **`ShorePointCard`** | "Shore point B-2, Wood Shore Secured, Division 2. Double tap to open." State is the *word*, not the stripe color. |
| **Status slide (advance / step-back)** | **Not a focus stop and not AT-operable** — pointer drag is the only commit path ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)). The track label is visible text ("Slide to set Runner"); a gated slide shows its reason as text under the track; every commit announces through the card's polite live region ("Shore point B-2, now Cutting Station"). |
...
| **Status badge** (read-only) | Read inline as its label word: "Cutting Station." Never announced as a color. |
| **Count badge** (read-only) | Spoken with its referent, never bare: "12 shore points, Cutting Station" — not "12." Not a focus stop; read as part of its host row / header. |
...
| **Multi-step step-indicator** (Phase G workflows) | "[Flow], step 2 of 4, Column mapping." Announced **politely on each step change**; the count is spoken as words ("step 2 of 4"), never "2/4," and the step's name follows. Registered by the **Setting up inventory** workflow ([#218](https://github.com/Vergo402/paratech-struts/issues/218)) for the validated import; reused by any later multi-step flow. Not a focus stop itself — it labels the wizard region. ([`09-workflows/05-setting-up-inventory.md`](../09-workflows/05-setting-up-inventory.md)) |

**Registry rule.** The primitive cascade's scripts are all in the table above, each following this grammar: `sheet` (#183), `modal` (#184), `badge` (#186), `button` (#187), `list` (#188), `input` (#189), `toggle` (#190), `segmented` (#191), `slider` (#192), `toast` (#193), `empty-state` (#194), `loading-state` (#195), `nested-checklist` (#196), `warning-gate` (#293 — the 14th file, added at the Phase E audit 2026-06-07 per matrix K-11), and `side-drawer` ([#306](https://github.com/Vergo402/paratech-struts/issues/306) — the **15th** file, added at the Phase F #217 gate 2026-06-09 per [ADR-019](../11-decisions/ADR-019-side-drawer-primitive.md)). (`slider`'s row is the **Status slide** above — pointer-only per [ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md); it registers what a reader *hears*, not an operable path.) This file is where the scripts are collected, not where their interaction models are invented.
...
- **Keyboard parity with every gesture.** Esc dismisses; arrow keys move between cards and within pickers; Enter / Space activates. The delegated Enter/Space handler that makes `role="button"` elements operable (`app.js:8756`) is the v3 mechanism v4 components inherit. No interaction is gesture-only — with the one recorded exception of the status slide ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md); see *Assistive tech cannot slide*), which deliberately has **no** keyboard commit.
...
- **A gesture as the only path.** Any slide/swipe without an equivalent focusable, labeled button locks out every assistive-tech and keyboard user — the point of *Assistive tech cannot slide*. The **status slide is the one recorded exception** ([ADR-026](../11-decisions/ADR-026-slide-only-status-commit.md)) — deliberate, scoped to status transitions, never a precedent for the next gesture.
...
2. **Power Select detection mechanism.** **RESOLVED 2026-07-01 (#372): the explicit Settings "Native controls" toggle is the only trigger — no screen-reader sniffing.** The web has no reliable "a screen reader is running" signal, and the best-effort heuristics that were floated are fragile and wrongly-firing; skipped deliberately. This is an operator opt-in, not the sole accessibility path — the custom pickers stay keyboard/SR-operable regardless, so a screen-reader user who never finds the toggle still gets an accessible control. When on, every picker variant (`InlineSegmented`, `BottomSheetPicker`, `FullScreenList`, `VisualGridPicker`) falls back to the OS-native `<select>` (`useNativeControls()` → `PowerSelect`).
