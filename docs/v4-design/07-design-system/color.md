...
Cards never use a drop shadow; elevation is **stroke + a 1pt top-edge inner highlight at 6% opacity** in light/dark. Shadows are reserved for sheets and modals (geometry/blur specified in [`motion.md`](motion.md) / the primitive files; colors here):
