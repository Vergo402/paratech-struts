...
- **Bottom sheets** (plate picker, action sheets) slide up from the bottom edge — `--motion-transition` — *origin: it comes from the edge it lives on.* Translates `translateY(100%) → translateY(0)`; the scrim fades `0 → 40%` simultaneously. iOS hardening carries from v3.5.1: `touch-action: pan-y` + `transform: translateZ(0)` on the sheet container.
- **Modal overlays** fade the scrim `0 → 40%` over `--motion-transition`; the modal card translates up 8pt at the same time (origin slightly below center, arrives at center) — *orientation.*
