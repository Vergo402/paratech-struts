...
- **The in-rubble role.** `RapidSOS` explicitly concedes it; `IAMResponding` goes silent after the status tap; `Tablet Command` is iPad-first and the phone is read-only-ish. Partial overlap exists with niche tools — DRR Rescue, for example, sizes wood shores from an opening measurement — but none covers the operational flow from arrival through cutting to secured. The team officer standing in structure gloves over a measurement is no one else's design center. (Axis: tactical-X, role.)
