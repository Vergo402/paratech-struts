...
FieldShore plots in the **upper right quadrant**: tactical operations focus crossed with doctrine fluent depth. The position is not a competitive accident. It follows directly from every Phase A decision. Principle 1 ("defer to doctrine, not invention") pins the Y position; the scope of an active structural collapse incident pins the X position. Level IV–V is the everyday case, roughly ninety-nine percent of structural collapse runs, but the app must scale alongside the operation as it grows. One team officer with two shore points on a phone is the starting state. A federal task force tracking two hundred fifty shore points on a broadcast board is the design ceiling. Every v4 interface decision must hold at both ends of that range. Every other principle (calm in chaos, one canonical action per state, visible safety, doubt free defaults, designed for the role, local first) is a consequence of those two commitments, not an independent dimension.
