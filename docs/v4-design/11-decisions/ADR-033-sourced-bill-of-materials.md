...
**Reviewer(s):** Alex — accepted 2026-06-15. The flagged product call is resolved: plate not-stocked = **quick-add, then deploy** (Alex's call); extensions follow the same quick-add model; disambiguation defaults to the strut's rig. **Refinement (Alex, 2026-06-15):** deploying a component that *stays* untracked (the operator declines quick-add) is permitted but **requires an explicit confirmation popup** — untracked deploy is always deliberate and flagged, never silent. **Refinement 2 (Alex, 2026-06-15):** any deploy whose BOM spans **more than one apparatus** must surface a **source-confirmation list** (each component + the rig it pulls from) and an explicit Confirm before commit — multi-rig sourcing is never silent, *even when no component is ambiguous*; only a single-rig BOM auto-deploys. Build is sequenced with the Inventory block ([#200](https://github.com/Vergo402/paratech-struts/issues/200)).
...
- **Inventory-consequential events commit one at a time** — `commitMany` explicitly rejects `StrutDeployed`/`StrutReturned` ("inventory-consequential events commit one at a time"). The group-aware step-back already honors this: `OperationsBoard` collects `stepBackMembers` (all Equipment Assigned members of a grouped shore) and `StepBackConfirmModal` commits **one `StrutReturned` per member** in a loop.
...
### 4. Assign Equipment UX — silent only for a single-rig shore; confirm every multi-rig pull; phone-first
...
- **Auto (single-rig BOM — the overwhelming common case):** if every component resolves to the **same one rig** (each stocked there with `available > 0`), source the whole BOM silently. A small department with one Rescue carrying everything sees **zero new friction** — tap Deploy, done, exactly as today. *Silent applies only when the entire pull is one rig.*
- **Confirm the multi-rig pull (whenever the BOM spans ≥2 rigs — Alex, 2026-06-15):** the moment the assembled BOM sources from **more than one apparatus**, a **"Source components" confirmation step** appears *inside the existing sheet* (not a new modal — ADR-016: deploy lives in the sheet), listing **every component and the rig it pulls from** (e.g. *strut — Rescue 2 · base plate — Engine 1*), and requires an explicit **Confirm** before commit. This fires **even when no component is ambiguous** — a two-rig pull is a coordination fact the crew must see and acknowledge (they physically stage from two trucks), not friction to hide. Where a component *is* ambiguous (>1 rig stocks the same part), its row is a rig segmented/select (defaulting to the rig the strut came from); unambiguous components show their single source read-only for awareness. The RecommendationCard's existing "from Rescue 2" line generalizes to this per-component source summary. This respects the preserved visual-grid plate **picker** (plate *selection* is untouched; this is plate *sourcing*, a separate rig fact).
...
Phone-first throughout: the source step is a vertical list of ≤5 rows (strut + ≤2 plates + ≤2 extensions), each either a read-only source line or a 56pt segmented/select when that component is ambiguous; it renders only when the pull spans ≥2 rigs, so the single-rig floor case never sees it.
...
- **Silent-for-single-rig honors "phone is the floor."** The confirmation appears *only* when the pull actually spans more than one rig (or a component is ambiguous). The common single-rig shore deploys in one tap, exactly as today; a multi-rig shore trades one tap for a sourcing fact the crew needs anyway (which trucks to stage from).
...
- **One global deploy event for a whole grouped shore (all 3 posts in one event).** Rejected: contradicts L-7's member-by-member group handling and the just-shipped group-aware step-back (which deliberately operates per Equipment Assigned member). Grouped members must remain independently deployable/reversible.
...
- **Negative (accepted costs):**
  ...
  - A new source-confirmation step in the deploy sheet — net-new UX. It shows on **any multi-rig pull** (not only model-ambiguity), so it is less rare than a pure-disambiguation step would be; the single-rig floor case still skips it entirely.
...
- **Verify:** component tests for (a) single-rig auto-source → one tap, no extra step; (b) two-rig ambiguity → source step renders, choice flows into the BOM; (b′) **two-rig BOM with no ambiguity → the source-confirmation list still renders and requires Confirm** (multi-rig is never silent); (c) not-stocked → quick-add path adds stock then deploys tracked; (c′) not-stocked → **Deploy-untracked requires the confirmation popup**, and on confirm records the component `source:'untracked'` with no decrement and the "untracked" flag (and a return skips it); (d) RecommendationCard renders multi-source summary + the untracked flag. Live-drive on the slice (preview MCP) once Phase 1 stock exists.
...
4. **Multi-rig pull is never silent — confirm + list every source (Alex, 2026-06-15).** Silent auto-deploy applies *only* when the whole BOM is one rig. The instant components source from ≥2 apparatus, the deploy shows a **source-confirmation list** (each component + its rig) and requires an explicit Confirm — **even with zero model-ambiguity**. Chosen over the lighter "just say it's multiple locations" notice: the itemized list tells the crew *which* rigs to stage from. Tightens item 3 (the disambiguation default still pre-fills the strut's rig within that same list).
...
- **Resolved product calls (Alex, 2026-06-15):** plate not-stocked = quick-add-then-deploy; extensions follow the same quick-add model; disambiguation defaults to the strut's rig; **any multi-rig BOM shows a source-confirmation list before commit (never silent)**.
