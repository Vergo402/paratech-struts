...
3. **Keep equipment spec dimensions EXACT internally; round only the final result.** The 3.4″ plate height is **not** displayed or computed as 3⅜″. Pre-rounding each spec before subtracting injects ±1/16″ per term and *accumulates* (four terms → up to ±1/4″ drift, possibly in the unsafe direction), and it silently contradicts the published manufacturer O&M value. Round once, at the end, downward.
...
- **Rounding specs accumulates unsafe error and lies about the manual.** Exact-in, round-once-out caps total rounding at a single controlled 1/8″ step and preserves the published 3.4″ figure. *(SME verdict Q3.)*
...
- **Positive:** the cut-to answer is legible at a gloved glance; output is honest about real field resolution; rounding is always safe-side; spec dimensions stay faithful to the manual; fraction rendering loses all codepoint hacks and is uniform across denominators.
